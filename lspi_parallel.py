...
def train_local_lspi(hospital_data):
    """Function to train a local LSPI model (for parallel execution)"""
    env = RealMedicalEnv(hospital_data)
    Q, _ = policy_iteration(env, 5, 2, 3)  # num_states=5, num_actions=2, iterations=3
    return Q

def federated_lspi(hospitals_data, num_rounds=5):
    """Parallel Federated LSPI implementation"""
    num_states = 5
    num_actions = 2
    ...
    for r in range(num_rounds):
        # Parallel execution across hospitals
        with Pool() as pool:
            local_Qs = pool.map(train_local_lspi, hospitals_data)
...
def compare_lspi(data_dir):
    ...
    print(f"\nTime taken - Federated: {fed_time:.2f}s | Centralized: {cen_time:.2f}s")
